...
export default function UploadPage() {
  ...
  const handleFiles = (e: React.ChangeEvent<HTMLInputElement>, isFolder = false) => {
    ...
    const newFiles: Asset[] = Array.from(files).map((file) => {
      ...
      let type: Asset["type"] = "text"; // default for all non-media files
  ...
  return (
    <div className="flex min-h-screen w-[25vw]">

      {/* Sidebar */}
      <aside className="w-[440px] p-5 flex flex-col gap-4 bg-white/95 border-r border-slate-100 shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          {filteredAssets.length === 0 ? (
            <p className="text-center text-slate-400 font-bold pt-10">No {activeTab.toLowerCase()} found</p>
          ) : (
