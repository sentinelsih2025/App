...
const getTheme = (level: string) => {
    ...
    if (normalizedLevel.includes("high") || normalizedLevel.includes("critical")) {
        ...
        return { color: "text-[#00ffa2]", border: "border-[#00ffa2]/50", bg: "bg-[#00ffa2]/10", glow: "shadow-[0_0_15px_rgba(0,255,162,0.15)]", bar: "bg-[#00ffa2]", button: "hover:bg-[#00ffa2]/20 border-[#00ffa2]/40 text-[#00ffa2]", status: "ANALYZING" };
    }
};

export default function TextAgent({ uploadTime = 0 }: { uploadTime?: number }) {
    ...
    return (
      <div className="flex items-center justify-center p-4 border border-[#00ffa2]/30 rounded-xl bg-[#000510]">
        <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-[#00ffa2] text-xs tracking-widest uppercase opacity-70 font-mono mt-1 animate-pulse">
                {isProcessing ? "Decrypting Stream..." : "Awaiting Text Intel..."}
            </p>
        </div>
      </div>
    );
